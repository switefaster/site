...
其中 $\pi$ 是构造点集 $T\mathcal{M}$ 时笛卡尔积的自然投影。令 $A$ 为 $T\mathcal{M}$ 的子集，我们令 $A$ 是 $T\mathcal{M}$ 的开集当且仅当 $\tilde{\phi}_\alpha(A\cap \pi^{-1}(U_\alpha))$ 对所有 $\alpha$ 都是 $\mathbb{R}^{2n}$ 中的开集。借此，我们构造了 $T\mathcal{M}$ 上的拓扑，也就是构造了全空间，而上述过程顺带构造了向量丛定义所需的同胚和丛投射。尽管这个同胚为逐个开集构造，流形坐标卡相容的性质保证了该构造是整体良定的。
...
\nabla_X(Y)=X\llcorner\nabla(Y)
$$
$\llcorner$ 指将 $X$ 与 $\Gamma(T^*\mathcal{M})$ 缩并。正文中符号的优势在于紧凑而且由于张量积的缘故自然有联络对于下指标 $C^\infty(\mathcal{M})$-线性。
:::
:::info[标架]
...
$$
我们同时引入了 $B$ 的坐标卡 $(x^\alpha)$。若将结果统一表示在基 $dx^\alpha\otimes e_i$ 下，则有（省略位置 $x$）
$$
...
$$
\nabla(X\wedge \alpha)=\nabla X\wedge \alpha+(-1)^{\mathrm{deg} X}X\wedge d\alpha\quad \alpha\in\Omega^1(B)
$$
:::
特别的，$\nabla$ 将向量值 $\Gamma(\Omega^p(B)\otimes E)$ 映射到向量值 $\Gamma(\Omega^{p+1}(B)\otimes E)$，可以证明其在标架下具有形式 $\nabla=d+A\wedge$。
::::
::::tip[平行与平行移动]
...
我们来考虑一个环形路径 $\gamma:[0,1]\to B$，即 $\gamma(0)=\gamma(1)$。若平移结果与路径无关，那么沿着环路的平移应该不导致向量的变化。按照平移的定义截面 $X$ 应满足 $\gamma^\prime(t)\llcorner(d+A)X=0$。因此我们需要解微分方程：
$$
\gamma^\prime(t)\llcorner dX=(\gamma^\prime)^\alpha\partial_\alpha X^i=-\gamma^\prime(t)\llcorner A^i{}_j X^j=-(\gamma^\prime)^\alpha\omega^i{}_{j\alpha} X^j
$$
...
:::
:::info[测地线]
若考虑的是某个流形的切丛上的平行移动，则由于切向量 $\gamma^\prime$ 也是定义联络的向量丛中的元素，我们可以构造一个非常特殊的平行移动方程，即测地线方程
$$
X^\alpha\partial_\alpha X^i=-X^\alpha\omega^i{}_{j\alpha} X^j
$$
可以理解成将无穷小平行移动后的切向量作为下一个平行移动的方向。因此通过对解上述方程得到的向量场积分，测地线方程可以生成流形上的一条曲线，即**测地线 (geodesic)**。测地线的特殊之处在于，在黎曼几何中，Levi-Civita 联络给出的两点之间的测地线正是两点之间的最短路径。

若考虑的流形为一个李群，则也可以以类似的思路构造平行移动，进而得到测地线。对于李群 $G$，其李代数的元素 $X\in\mathfrak{g}$ 正是单位元处的切向量 $X\in T_eG$。由于李群上自然存在一系列自同构 $L_g(g^\prime)=gg^\prime$，我们可以通过 $(L_{g})_*X$ 将这一向量推前至任意群元 $g$ 处的向量，这被称为左作用，而如此构造的向量场便被称为左不变向量场。对于 $n$ 维的李群，我们可以对其 $n$ 个生成元分别构造一个左不变的矢量场：很显然它们构成李群切丛的一个标架 $(T_i)$。我们可以自然构造一个联络
$$
\nabla(T_i)=0
$$
很显然这个联络也是左不变的，且由构造显然是平坦的。我们可以通过这个联络生成测地线 $\gamma_X(s)$，容易证明
$$
\begin{cases}
\gamma_X(s_1+s_2)=\gamma_X(s_1)\gamma_X(s_2)=\gamma_X(s_2)\gamma_X(s_1)\\
\gamma_X(s)=\gamma_{sX}(1)
\end{cases}
$$
即 $\gamma_X$ 生成了李群 $G$ 的一个单参数阿贝尔子群。因此 $\exp(X)=\gamma_X(1)$ 形成了一个 $\mathfrak{g}\mapsto G$ 的映射，且可以证明其在 $G$ 的连通子群上是满射。这个映射也被称为**指数映射 (exponential map)**。

若考虑的李群为矩阵群，或者考虑李群的表示，则该联络的作用更加直接，即写作
$$
\frac{dA}{d\tau}=-TA
$$
其中 $A$ 为群元，$T$ 为李群的某个生成元。由于左不变性，$T$ 与 $\tau$ 无关，可以直接积分得到 $A=A(0)\exp\{-\tau T\}$。其在物理中最熟悉的一个例子为薛定谔方程，可以得到时间演化算符 $U(t,t_0)=\exp\{-iH(t-t_0)\}$。这也正是指数映射名称的来源。

容易注意到该方程的形式与 $dX=-AX$ 十分相似，其中关系对应留作读者思考。

出于二者相似的构造，在黎曼几何中通过某点切向量的测地线得到 $T_p\mathcal{M}\to U\subset\mathcal{M}$ 的映射也被称为指数映射。
:::
:::info[和乐]
...
定义联络的限制 $\varphi\circ\nabla|_V:\Gamma(\mathcal{R}\times V)\to\Gamma(T^*\mathcal{R}\otimes\mathcal{R}\times V)$，其中 $\varphi$ 为某种典范的线性投影。
...
然而，在切丛的语境下，由于天然存在一个好的标架 $(\partial_\mu)$，以及种种历史原因，很多教材在初学时会忽略切丛上可以任选标架的事实，而直接在 $(\partial_\mu)$ 中定义所有量。例如经典的克氏符（与正文中比，交换了两个下指标）
$$
...
Levi-Civita 联络的想法如下：使平行移动保持度规，例如始相互正交的向量在同时平移后依旧是相互正交的。该要求等价于
$$
\nabla g=0
$$
