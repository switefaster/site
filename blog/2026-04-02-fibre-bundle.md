...
事实上，前面引言所述内容有一点点误导性。很显然，贝里相位按其定义是独立定义在每一个本征态上的，或者说每一个本征子空间都是贝里联络的不变子空间：贝里联络分块对角地定义在每一个本征子空间上，它们的直和定义了总的希尔伯特空间为纤维的丛的联络。这一想法非常重要，而我们在这一小节将反其道而行之：我们先考虑平凡丛 $\mathcal{R}\times\mathcal{H}$，并说明其上自然存在一个平凡的联络，而这个联络是具有我们期待的物理含义的。
...
:::
我们可以借由度规唯一确定 Levi-Civita 联络。在本文我们不多赘述。
